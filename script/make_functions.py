...
def modify_config_1core_l1_prefetcher(branch, prefetcher, name):
    with open("./champsim_config.json", "r") as f:
        load_dict = json.load(f)

    load_dict['ooo_cpu'][0]['branch_predictor'] = branch
    load_dict['L1D']['prefetcher'] = prefetcher
    load_dict['executable_name'] = name

    load_dict['L1D']['virtual_prefetch'] = False
...
def make_1core_l1_prefetcher(prefetcher):
    name = f'champsim_1core_{prefetcher}'
    modify_config_1core_l1_prefetcher('tage-sc-l', prefetcher, name)
    os.system('./config.sh ./champsim_config_auto.json')
    os.system('make clean && make -j16')

def modify_config_1core_l2_prefetcher(branch, prefetcher, name):
    with open("./champsim_config.json", "r") as f:
        load_dict = json.load(f)

    load_dict['ooo_cpu'][0]['branch_predictor'] = branch
    load_dict['L1D']['prefetcher'] = 'berti'
...
def make_1core_l2_prefetcher(prefetcher):
    name = f'champsim_1core_{prefetcher}'
    modify_config_1core_l2_prefetcher('tage-sc-l', prefetcher, name)
    os.system('./config.sh ./champsim_config_auto.json')
    os.system('make clean && make -j16')

def modify_config_1core_l1_prefetcher_multi_level(branch, prefetchers, name):
    with open("./champsim_config.json", "r") as f:
        load_dict = json.load(f)
...
def make_1core_multi_level(prefetchers):
    ...
    modify_config_1core_l1_prefetcher_multi_level('hashed_perceptron', prefetchers, name)
    os.system('./config.sh champsim_config_auto.json')
    os.system('make clean && make -j16')
    
    
def modify_config_1core_l1_prefetcher_system_sensitivity(branch, prefetcher, bw, llc_size, l2c_size, name):
...
def make_1core_system_sensitivity(prefetcher):
    ...
    for bw in [800, 1600, 6400, 12800]:
        ...
        modify_config_1core_l1_prefetcher_system_sensitivity('hashed_perceptron', prefetcher, bw, llc_size, l2c_size, name)
        os.system('./config.sh champsim_config_auto.json')
        os.system('make clean && make -j16')
