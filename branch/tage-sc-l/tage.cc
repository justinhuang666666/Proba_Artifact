...
struct ChampsimTageScl {
  ...
  ChampsimTageScl(const ChampsimTageScl&) = delete;
  ChampsimTageScl& operator=(const ChampsimTageScl&) = delete;
  ChampsimTageScl(ChampsimTageScl&&) = delete;
  ChampsimTageScl& operator=(ChampsimTageScl&&) = delete;
...
static std::vector<std::unique_ptr<ChampsimTageScl>> predictors;
...
void O3_CPU::initialize_branch_predictor() {
  cpus.push_back(this);
  predictors.push_back(std::make_unique<ChampsimTageScl>(1));
}
