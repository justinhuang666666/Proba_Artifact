...
ActiveGenerationTable::Entry* ActiveGenerationTable::set_pattern(uint64_t region_num, uint64_t offset) {
    ...
    if (!entry)
        ...
        if(entry->data.second_offset==-1)entry->data.second_offset=offset;
        Super::rp_promote(key);
        return entry;
    }
}
...
ActiveGenerationTable::Entry ActiveGenerationTable::insert(uint64_t region_num, uint64_t trigger_offset, uint64_t pc, uint64_t addr) {
    ...
    Entry old_entry = Super::insert(key, {trigger_offset, -1, pc, addr, pattern});
    Super::rp_insert(key);
    return old_entry;
}
...
void PatternHistoryTable::insert(uint64_t pc, uint64_t offset, uint64_t offset2, uint64_t addr, const std::vector<int> &pattern) {
    assert((int)pattern.size() == proba::NUM_BLOCKS);
    uint64_t key = build_key(pc,offset,offset2,addr);
...
void PatternHistoryTable::update(uint64_t pc, uint64_t offset, uint64_t offset2, uint64_t addr, const std::vector<int> &pattern, const custom_util::SaturatingCounter &mode) {
    assert((int)pattern.size() == proba::NUM_BLOCKS);
    uint64_t key = build_key(pc,offset,offset2,addr);
    // std::cout<<"pc key: "<<std::hex<<key<<std::dec<<std::endl;
...
PatternHistoryTable::Entry* PatternHistoryTable::find(uint64_t pc, uint64_t offset, uint64_t offset2, uint64_t addr) {
    if(offset2 ==-1 && behavior == PatternHistoryTable::Behavior::OffsetOffset) return nullptr;
    uint64_t key = build_key(pc,offset,offset2,addr);
    Entry* entry = Super::find(key);
    if (entry)
        Super::rp_promote(key);
...
// }
uint64_t PatternHistoryTable::build_key(uint64_t pc, uint64_t offset, uint64_t offset2, uint64_t addr) {
    uint64_t base_key = 0;
    ...
    case PatternHistoryTable::Behavior::PCOffset:
        base_key = custom_util::folded_xor(pc, 2);
        base_key = (base_key << proba::OFFSET_WIDTH) | offset;
        // Use PC + offset
        break;
    ...
    case PatternHistoryTable::Behavior::OffsetOffset:
        base_key = (offset2 << proba::OFFSET_WIDTH) | offset;
...
void Proba::access(uint64_t block_num, uint64_t pc, CACHE* cache) {
    ...
    if(!agt_entry || offset2 != -1) {
        if(offset2 != -1 || !this->jt.in_jail(region_num)||!use_jail_table) {
            if (is_debug) {
            ...
            // Loop over all PHTs
            for (auto& table : phts) {
                if(offset2 !=-1 && table.behavior != PatternHistoryTable::Behavior::OffsetOffset) continue;
                if(offset2 ==-1 && table.behavior == PatternHistoryTable::Behavior::OffsetOffset) continue;
                auto entry = table.find(pc, region_offset,offset2,block_num);
                if (entry) {
                    if (is_debug) {
                        std::cout << "Pattern from table: "
                                  << custom_util::pattern_to_string(entry->data.pattern)
                                  << std::endl;
                    }

                    if (accumulated_pattern.empty()) {
                        accumulated_pattern = entry->data.pattern; // first table
                    } else {
                        auto rotated_pattern = table.behavior == PatternHistoryTable::Behavior::PC? rotate(entry->data.pattern, region_offset) :entry->data.pattern;
                        // Accumulate union element-wise
                        for (size_t i = 0; i < accumulated_pattern.size(); ++i) {
                            accumulated_pattern[i] |= entry->data.pattern[i];
                        }
                    }
                }
            }
...
void Proba::update_in_pht(const ActiveGenerationTable::Entry& agt_entry, bool is_end_of_generation, CACHE* cache) {
    ...
    if(!use_only_training_on_eog || is_end_of_generation) {
        ...
        uint64_t pop_count_observation = safe_minus_one(count_bits_set(observation));
        uint64_t pop_count_prediction = safe_minus_one(count_bits_set(accumulated_prediction));
        uint64_t same_count_observation_prediction = safe_minus_one(count_bits_same(accumulated_prediction, observation));
        ...
        for (size_t idx : indices) {
            ...
            if(first_iteration) {
                ...
                uint64_t pop_count_prediction = safe_minus_one(count_bits_set(observation));
