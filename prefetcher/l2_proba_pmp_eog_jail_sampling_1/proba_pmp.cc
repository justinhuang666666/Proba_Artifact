...
PCPatternTable::PCPatternTable(int size, int num_ways, int pc_width) :
    Super(size, num_ways), pc_width(pc_width) {
    std::cout << "PC Pattern Table index_len: " << Super::index_len << std::endl;
}

void PCPatternTable::insert(uint64_t pc, const std::vector<int> &pattern) {
...
void PCPatternTable::update(uint64_t pc, const std::vector<int> &pattern, const custom_util::SaturatingCounter &mode) {
    assert((int)pattern.size() == proba::NUM_BLOCKS);
    uint64_t key = build_key(pc);
    // std::cout<<"pc key: "<<std::hex<<key<<std::dec<<std::endl;
...
PCPatternTable::Entry* PCPatternTable::find(uint64_t pc) {
    uint64_t key = build_key(pc);
    Entry* entry = Super::find(key);
    if (entry)
        Super::rp_promote(key);
...
uint64_t PCPatternTable::build_key(uint64_t pc) {
...
Proba::Proba(int agt_size, int agt_ways, int opt_size, int opt_ways, int offset_width, int ppt_size, int ppt_ways, int pc_width, int jt_size, int pb_size, int pb_ways, bool is_debug, int cpu) :
    agt(agt_size, agt_ways), opt(opt_size, opt_ways, offset_width), ppt(ppt_size, ppt_ways, pc_width), jt(jt_size), pb(pb_size, NUM_BLOCKS, 0, pb_ways), is_debug(is_debug), cpu(cpu) {
}
...
void Proba::update_in_ppt(const ActiveGenerationTable::Entry& agt_entry, bool is_end_of_generation, CACHE* cache) {
    if(!use_only_training_on_eog || is_end_of_generation){
        auto ppt_entry = ppt.find(agt_entry.data.pc);
        if(ppt_entry){
            if (is_debug) std::cout << "PHT entry found" << std::endl;
            ...
            ppt.update(agt_entry.data.pc, prediction, mode);
        } else {
            if(count_bits_set(pattern_bool2int(agt_entry.data.pattern))>1){
                if (is_debug) std::cout << "PHT entry not fount, insert new PHT entry" <<std::endl;
                ppt.insert(agt_entry.data.pc, rotate(pattern_bool2int(agt_entry.data.pattern), -agt_entry.data.trigger_offset));
            }
        }
    }
...
void CACHE::prefetcher_initialize() {
    ...
    prefetchers = std::vector<proba::Proba>(NUM_CPUS, proba::Proba(proba::AGT_SIZE, proba::AGT_WAY, proba::OPT_SIZE, proba::OPT_WAY, proba::OFFSET_WIDTH, proba::PPT_SIZE, proba::PPT_WAY, proba::PC_WIDTH, proba::JT_SIZE, proba::PB_SIZE, proba::PB_WAY, proba::DEBUG, cpu));
}
