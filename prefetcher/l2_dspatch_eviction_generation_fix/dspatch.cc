...
const char* DSPatch_pref_candidate_string[] = {"PAT_NONE", "CovP", "AccP"};
...
constexpr int dspatch_sig_type = 1;
constexpr int dspatch_sig_hash_type = 2;
constexpr int dspatch_or_count_max = 3;
constexpr int dspatch_measure_covP_max = 3;
constexpr int dspatch_measure_accP_max = 3;
constexpr int dspatch_acc_thr = 50;
constexpr int dspatch_cov_thr = 50;
constexpr int dspatch_enable_pref_buffer = true;
constexpr int dspatch_pref_buffer_size = 256;
constexpr int dspatch_pref_degree = 4;
} // namespace knob
...
DSPatch::DSPatch(string type) :
    Prefetcher(type), pb(32, knob::dspatch_num_cachelines_in_region) {
    init_knobs();
    init_stats();
...
void DSPatch::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<uint64_t>& pref_addr) {
    ...
    } else /* page buffer miss, prefetch trigger opportunity */
    {
        ...
        /* trigger prefetch */
        generate_prefetch(pc, page, offset, address);
...
void DSPatch::erase(uint64_t page_num) {
    for (auto it = page_buffer.begin(); it != page_buffer.end(); ++it) {
        if ((*it) != nullptr && (*it)->page == page_num) {
...
void DSPatch::generate_prefetch(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address) {
    ...
    bmp_pred = BitmapHelper::decompress(bmp_pred, knob::dspatch_compression_granularity, knob::dspatch_num_cachelines_in_region/knob::dspatch_compression_granularity);
    ...
    uint64_t region_num = page;
    std::vector<int> pattern(knob::dspatch_num_cachelines_in_region, 0);
    for (uint32_t index = 0; index < knob::dspatch_num_cachelines_in_region; ++index) {
        if (bmp_pred[index] && index != offset) {
            pattern[index] = 1;
        }
    }
    pb.insert(region_num, pattern);
    // for (uint32_t index = 0; index < knob::dspatch_num_cachelines_in_region; ++index) {
    //     if (bmp_pred[index] && index != offset) {
    ...
    //     }
    // }
    stats.gen_pref.total += std::count_if(pattern.begin(), pattern.end(), [](auto x) { return x; });
}
...
uint32_t DSPatch::get_spt_index(uint64_t signature) {
    uint32_t folded_sig = custom_util::folded_xor(signature, 2);
    /* apply hash */
...
void DSPatch::add_to_spt(DSPatch_PBEntry* pbentry) {
    ...
    bmp_cov = BitmapHelper::decompress(sptentry->bmp_cov, knob::dspatch_compression_granularity, knob::dspatch_num_cachelines_in_region/knob::dspatch_compression_granularity);
    bmp_acc = BitmapHelper::decompress(sptentry->bmp_acc, knob::dspatch_compression_granularity, knob::dspatch_num_cachelines_in_region/knob::dspatch_compression_granularity);
    ...
    sptentry->bmp_acc = BitmapHelper::bitwise_and(bmp_real, BitmapHelper::decompress(sptentry->bmp_cov, knob::dspatch_compression_granularity, knob::dspatch_num_cachelines_in_region/knob::dspatch_compression_granularity));
...
void DSPatch::eviction(uint64_t block_num) {
    ...
    uint32_t region_offset = block_num & ((1ull << (knob::dspatch_log2_region_size - LOG2_BLOCK_SIZE)) - 1);

    auto entry = search_pb(page_num);
    if (entry && entry->bmp_real[region_offset]) {
        add_to_spt(entry);
        erase(page_num);
    }
}

int DSPatch::prefetch(CACHE* cache, uint64_t block_num) {
    int pf_issued = pb.prefetch(cache, block_num);
    return pf_issued;
}

namespace {
vector<DSPatch> prefetchers;
} // namespace
...
void CACHE::prefetcher_cycle_operate() {
    prefetchers[cpu].update_bw(bw);
}

void CACHE::prefetcher_final_stats() {
    prefetchers[cpu].dump_stats();
}
