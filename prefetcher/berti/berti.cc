...
// std::map<uint64_t, int> ip_map;

// std::map<uint64_t, std::vector<int>> loaded_regions, prefetched_regions;

namespace berti_space {
std::vector<int> delta_counter(1 << DELTA_MASK, 0);

static std::vector<LatencyTable> latencyts;
static std::vector<ShadowCache> scaches;
static std::vector<HistoryTable> historyts;
static std::vector<Berti> bertis;
}; // namespace berti_space
/******************************************************************************/
    ...
    for (int i = 0; i < sets; i++) {
        for (int ii = 0; ii < ways; ii++) {
            if (scache[i][ii].addr == addr) {
                if constexpr (champsim::debug_print) {
                    std::cout << " set: " << i << " way: " << ii << std::endl;
                }
                return true;
            }
        }
...
void Berti::find_and_update(uint32_t cpu, uint64_t latency, uint64_t tag, uint64_t cycle, uint64_t line_addr) {
    ...
    num_on_time = historyts[cpu].get(latency, tag, line_addr, tags, addr, cycle);
...
void CACHE::prefetcher_initialize() {
    ...
    uint64_t latency_table_size = get_size(0, 0);
    for (auto i = 0; i < get_size(1, 0); i++) latency_table_size += i;
    for (auto i = 0; i < get_size(2, 0); i++) latency_table_size += i;
    for (auto i = 0; i < get_size(3, 0); i++) latency_table_size += i;

    // New structures
    latencyts = std::vector<LatencyTable>(NUM_CPUS, (LatencyTable(latency_table_size)));
    scaches = std::vector<ShadowCache>(NUM_CPUS, (ShadowCache(this->NUM_SET, this->NUM_WAY)));
    historyts = std::vector<HistoryTable>(NUM_CPUS, (HistoryTable()));
    bertis = std::vector<Berti>(NUM_CPUS, (Berti(BERTI_TABLE_DELTA_SIZE)));

    std::cout << "Berti SOTA Prefetcher" << std::endl;
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in) {
    ...
    uint64_t ip_hash = bertis[cpu].ip_hash(ip) & IP_MASK;
    ...
    if (!cache_hit) // This is a miss
    {
        ...
        latencyts[cpu].add(line_addr, ip_hash, false, current_cycle); // Add @ to latency
        historyts[cpu].add(ip_hash, line_addr, current_cycle);        // Add to the table
    } else if (cache_hit && scaches[cpu].is_pf(line_addr))            // Hit bc prefetch
    {
        if constexpr (champsim::debug_print)
            std::cout << "[BERTI] operate cache hit because of pf" << std::endl;

        scaches[cpu].set_pf(line_addr, false);

        uint64_t latency = scaches[cpu].get_latency(line_addr); // Get latency

        if (latency > LAT_MASK)
            latency = 0;

        bertis[cpu].find_and_update(cpu, latency, ip_hash, current_cycle & TIME_MASK, line_addr);
        historyts[cpu].add(ip_hash, line_addr, current_cycle & TIME_MASK);
    } else {
        if constexpr (champsim::debug_print)
    ...
    for (auto i : deltas) {
        ...
        float mshr_load = (float)get_occupancy(0, 0) / get_size(0, 0) * 100;
        ...
        if (prefetch_line(p_addr, fill_this_level, pf_metadata)) {
            ++average_issued;
            if (first_issue) {
                first_issue = false;
                ++average_num;
            }
            delta_counter[abs(i.delta)]++;
            ...
            if (fill_this_level) {
                if (!scaches[cpu].get(p_b_addr)) {
                    latencyts[cpu].add(p_b_addr, ip_hash, true, current_cycle);
                }
            }
        }
    }
...
uint32_t CACHE::prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in) {
    uint64_t line_addr = (addr >> LOG2_BLOCK_SIZE); // Line addr
    uint64_t tag = latencyts[cpu].get_tag(line_addr);
    uint64_t cycle = latencyts[cpu].del(line_addr) & TIME_MASK;
    uint64_t latency = 0;
    ...
    scaches[cpu].add(set, way, line_addr, prefetch, latency);

    if (latency != 0 && !prefetch) {
        bertis[cpu].find_and_update(cpu, latency, tag, cycle, line_addr);
    }
    return metadata_in;
}

void CACHE::prefetcher_final_stats() {
    int sum_delta_less_64 = 0, sum_delta_larger_64 = 0;
    for (int i = 0; i < (1 << DELTA_MASK); i++) {
        // if (berti_space::delta_counter[i] > 0 && i < 64)
        //     sum_delta_less_64 += berti_space::delta_counter[i];
        // else
        //     sum_delta_larger_64 += berti_space::delta_counter[i];
        std::cout << "delta: " << i << ", " << berti_space::delta_counter[i] << std::endl;
    }

    std::cout << "sum_delta_less_64: " << sum_delta_less_64 << ", sum_delta_larger_64: " << sum_delta_larger_64 << std::endl;
    ...
    std::cout << "BERTI CROSS_PAGE_ISSUED " << cross_page_issued;
    std::cout << " NO_CROSS_PAGE_ISSUED: " << no_cross_page_issued << std::endl;
