...
public:
    Bingo(int pattern_len, int min_addr_width, int max_addr_width, int key_width, int filter_table_size,
          int accumulation_table_size, int pht_size, int pht_ways, int jt_size, int pb_size, int pb_way, int debug_level = 0) :
    ...
    void eviction(uint64_t block_number) {
        ...
        if (entry) {
            /* move from accumulation table to pattern history table */
            this->update_in_phts(*entry, true);
        } else {
            this->jail_table.unmark(region_number);
        }
    }
